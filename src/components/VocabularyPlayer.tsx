...
export function VocabularyPlayer() {
  ...
  return (
    <div className="relative min-h-screen bg-gray-100 flex flex-col"
        style={{ backgroundImage: 'url(/images/english.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}
    >
      <div className="flex justify-between items-center p-4 bg-white/80 shadow-md z-10">
        <div className="text-lg font-semibold">
          <span className="mr-2">
            {settingsVariables[0].options.find(g => g.value === grade)?.label}
          </span>
          <span>
            {settingsVariables[1].options.find(s => s.value === semester)?.label}
          </span>
        </div>
